...
def clean_text(text: str) -> str:
    """
    Strips extra whitespace, remove stop words, and lemmatizes text.
...
def plot_confusion_matrix(confusion_matrix, class_names, errors_only=False,
    ...
    conf_matrix_norm = confusion_matrix.astype('float') / confusion_matrix.sum(axis=1)[:,
                                                          np.newaxis]
    ...
    heatmap = sns.heatmap(df_cm_norm, ax=ax1, cmap='Blues', fmt='.3f', annot=True,
    ...
    ax1.set_title('Confusion Matrix - Normalized', pad=15, fontsize=fontsize,
                  color='black')

    # ax2 - Confusion Matrix - Class Counts
    df_cm = pd.DataFrame(confusion_matrix, index=class_names, columns=class_names)
    heatmap = sns.heatmap(df_cm, ax=ax2, cmap='Blues', fmt='d', annot=True,
    ...
    ax2.set_title('Confusion Matrix - Class Counts', pad=15, fontsize=fontsize,
                  color='black')
